...
// v1.2：入参为步骤元数据（StepRecord 结构兼容 ExecutionStep，无需快照）
import type { FlowEvent } from '../core/steps';
 ...
 * now  = records[currentStep] 的全部事件。
 * 两者按 step 边界划分：同一事件不会同时出现在 past 与 now（修复旧实现
 * `history.slice(0, -1)` 在当前步含 0 个事件时丢事件、含多个事件时重复的问题）。
 */
export function splitFlowEvents(
  records: readonly { flowEvents: FlowEvent[] }[],
  currentStep: number,
): { past: FlowEvent[]; now: FlowEvent[] } {
  const past: FlowEvent[] = [];
  const from = Math.max(0, currentStep - FLOW_HISTORY_STEPS);
  for (let i = from; i < currentStep && i < records.length; i++) {
    past.push(...records[i].flowEvents);
  }
  const cur = records[currentStep];
